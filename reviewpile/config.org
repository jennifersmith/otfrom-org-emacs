...
(defun goto-config ()
"Jump to the config file for emacs"
(interactive)
(find-file "~/.emacs.d/org/config.org"))

(defun goto-local-config ()
"Jump to the local/customize config file for emacs"
(interactive)
(find-file "~/.emacs.d/local/custom.el"))
...
do etags stuff from emacs
#+BEGIN_SRC emacs-lisp
  (defun create-tags (dir-name)
     "Create tags file."
     (interactive "DDirectory: ")
     (eshell-command
      (format "cd %s; git ls-files | grep -ve 'png$' |  | etags -" dir-name)))
#+END_SRC
...
Update: this is not actually true any more. Going to manage the list
of relevant files in my local config !
...
(add-hook 'org-finalize-agenda-hook
          (lambda ()
            (save-excursion
              (color-org-header "PublicHolidays:"  "yellow1")
              (color-org-header "AnnualLeave:"  "yellow1")
              (color-org-header "Weekend:"  "yellow1")
              (color-org-header "WorkDay:"  "firebrick3")
               )))
...
(shell-command-to-string
  (concat "git commit -am 'Finishing review for " (format-time-string "%F") "'"))
(run-agenda-store))
#+END_SRC
...
Using a LOT now - so shortcut it
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
** epic
Evernote picker / exporter

#+BEGIN_SRC emacs-lisp

(maybe-install-and-require 'epic)
...
#+BEGIN_SRC emacs-lisp
  ...
  (maybe-install-and-require 'org-ref)
...
Borrowed from a gh that doesn'tseem to be there no more
#+BEGIN_SRC emacs-lisp
